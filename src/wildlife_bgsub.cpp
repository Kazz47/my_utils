...
std::ofstream variance_file; std::ofstream vibe_file;
...
void processVideo(std::string vid_filename, std::string event_filename) {
    ...
    while(capture.read(frame)){
        ...
        tsv_file << event_occur << "\t" << vibe_mean << "\t" << mog_mean << std::endl;
    }
    capture.release();
    ...
    double vibe_stdev = sqrt(calcVariance(vibe_vals, vibe_mean));
    double mog_stdev = sqrt(calcVariance(mog_vals, mog_mean));

    double vibe_matches = 0;
    double mog_matches = 0;
    for (double time : *event_times) {
        bool vibe_match = false;
        bool mog_match = false;
        for (int i = time - 20; i < time + 20; i++) {
            if (i >= 0 && i < vibe_vals.size() && !vibe_match && vibe_vals.at(i) > vibe_mean + vibe_stdev) {
                vibe_matches++;
                vibe_match = true;
            }
            if (i >= 0 && i < mog_vals.size() && !mog_match && mog_vals.at(i) > mog_mean + mog_stdev) {
                mog_matches++;
                mog_match = true;
            }
            if (vibe_match && mog_match) {
                break;
            }
        }
    }
    LOG(INFO) << "ViBe accuracy: " << vibe_matches/event_times->size();
    LOG(INFO) << "MOG accuracy: " << mog_matches/event_times->size();
}
